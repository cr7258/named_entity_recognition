from utils import load_model, extend_maps, prepocess_data_for_lstmcrf,generate_resume_list_from_txt, format_res
...
def main():
    ...
    isCustomized = True
    if isCustomized:
        test_word_lists, test_tag_lists = generate_resume_list_from_txt("data_for_test.txt")
        train_word_lists, train_tag_lists = generate_resume_list_from_txt("data_for_train.txt")
    ...
    hmm_pred = hmm_model.test(test_word_lists,
                              ...
                              tag2id)
    # print("解析结果：", hmm_pred)
    formatted_w, formatted_t = format_res(test_word_lists, hmm_pred)
    print("测试字符: ", test_word_lists)
    print("组合字符：", formatted_w)
    print("解析标签: ", hmm_pred)
    print("标签组合：", formatted_t)
